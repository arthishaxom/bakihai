...
- The Confirm action belongs to the Member whose key binds the receiver's id: the app offers it by that binding rather than by the roster's holder classification, so even a pair inverted by id order (ADR-0020) is still offered the Confirm on the phone that holds the key, and that phone's waiting line names itself rather than the fold's guess. A Settlement the holder records to their own shadow starts confirmed.
